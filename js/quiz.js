...
function checkCount() {
    ...
    if (askedCount == totalQuestion) {
        setTimeout(function() {
            ...
            // Retrieve the last high score from local storage
            let highScore = localStorage.getItem('highScore') || 0;

            // Check if the current score is higher than the stored high score
            if (correctScore > highScore) {
                localStorage.setItem('highScore', correctScore); // Update high score in local storage
                alert('New High Score!'); // Notify the player if they set a new high score
            }
